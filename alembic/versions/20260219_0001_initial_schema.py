...
def upgrade() -> None:
    ...
    if "start_message_cache" not in existing_tables:
        op.create_table(
            "start_message_cache",
            sa.Column("learner_id", sa.String(), nullable=False),
            sa.Column("template_id", sa.String(), nullable=False),
    ...
    op.execute("CREATE INDEX IF NOT EXISTS idx_lesson_cache_updated ON lesson_cache (updated_at DESC)")
    op.execute("CREATE INDEX IF NOT EXISTS idx_start_message_cache_updated ON start_message_cache (updated_at DESC)")
