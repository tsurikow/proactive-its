...
class StageSourceService:
    def __init__(self, store: AsyncVectorStore):
        self.store = store

    async def resolve(self, stage: dict[str, Any]) -> StageSource:
        ...
        parent = await self.store.fetch_section_parent(section_id)
        if not parent and module_id and module_id != section_id:
            parent = await self.store.fetch_section_parent(module_id)
        if not parent:
            raise RuntimeError(
